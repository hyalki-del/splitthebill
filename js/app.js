 ...
 * Architecture: Keyframe Motion Engine + Typographic Hierarchy + Persistent Metadata
 */
...
// --- Internationalization Dictionary with Typographic Hierarchy ---
const TRANSLATIONS = {
    en: {
...
function initTaglineCarousel() {
    ...
    function cycleTagline() {
        ...
        // Assign a random 4-directional keyframe animation class
    ...
    cycleTagline(); // First trigger
    taglineTimer = setInterval(cycleTagline, 3200); // Cycle every 3.2 seconds
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    render();
            ...
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Spend simply.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">Enjoy the moment. Leave tracking to SPENSE.</span>`,
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Just add what you spent.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">Who paid? Who shares? SPENSE does the math.</span>`,
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Settle easily.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">See exactly who owes whom — and how much.</span>`
        ]
    },
    tr: {
        settingsBtn: "⚙ Ayarlar", shareLinkBtn: "Bağlantıyı Paylaş", deleteBtn: "Sil",
        ...
        shareLinkHeader: "Defter Bağlantısını Paylaş", shareLinkSub: "Bu bağlantıya sahip herkes PIN girmelidir.", copyBtn: "Kopyala",
        taglines: [
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Kolayca harca.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">Anın tadını çıkar. Masraf takibini SPENSE'e bırak.</span>`,
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Sadece harcamanı ekle.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">Kim ödedi? Kimler paylaşıyor? Matematik işini SPENSE yapar.</span>`,
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Rahatça hesabı kapat.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">Kimin kime ne kadar borcu var — anında gör.</span>`
        ]
    },
    de: {
        settingsBtn: "⚙ Einstellungen", shareLinkBtn: "Link Teilen", deleteBtn: "Löschen",
        ...
        shareLinkHeader: "Teilen", shareLinkSub: "PIN erforderlich.", copyBtn: "Kopieren",
        taglines: [
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Einfach ausgeben.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">Genieße den Moment. Überlasse die Spesenverfolgung SPENSE.</span>`,
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Einfach eintragen.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">Wer hat bezahlt? Wer teilt es? SPENSE macht die Rechnung.</span>`,
            `<strong class="block font-black text-slate-900 text-2xl sm:text-3xl leading-tight">Einfach abrechnen.</strong><span class="block text-slate-600 text-xs sm:text-sm font-medium mt-1.5">Sehen Sie genau, wer wem schuldet — und wie viel.</span>`
        ]
    }
};
...
function initTaglineCarousel() {
    ...
    spot.innerHTML = activeTaglines[currentTaglineIndex % activeTaglines.length];
    spot.className = "w-full text-center leading-snug slide-reset";

    function rotateTagline() {
        const dir = directions[Math.floor(Math.random() * directions.length)];

        spot.className = "w-full text-center leading-snug " + dir.outClass;

        setTimeout(() => {
            currentTaglineIndex = (currentTaglineIndex + 1) % activeTaglines.length;
            spot.innerHTML = activeTaglines[currentTaglineIndex];
            
            spot.className = "w-full text-center leading-snug " + dir.inClass;
