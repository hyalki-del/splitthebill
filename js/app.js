...
function openWelcomeModal() {
    try {
        ...
        // Fetch archives safely without blocking UI execution
...
// Atomic Settings Mutation: Reads Language, Currency & Theme simultaneously
async function saveSettings() {
    const curEl = document.getElementById('settingsCurrencySelect');
    const newCur = curEl ? curEl.value : currentCurrency;
    ...
    // Apply state mutations immediately locally
    currentCurrency = newCur;
    currentLang = newLang;
    ...
    // Persist all three properties in a single API call
    await sendAction({ 
        action: "updateSettings", 
        currency: newCur, 
        theme: newTheme,
...
async function initApp() {
    ...
    try {
        ...
        console.warn("Could not load config.json, using fallback API_URL.");
    }
...
function closeWelcomeModal() { 
    if (!currentTab) return; // Prevent closing modal if no ledger is loaded
    const modal = document.getElementById('welcomeModal');
    if (modal) modal.classList.add('hidden'); 
}
...
async function initApp() {
    ...
    // Try reading config.json gracefully without breaking execution on failure
    try {
        const configRes = await fetch('config.json');
        if (configRes.ok) {
            const config = await configRes.json();
...
async function createNewLedger() {
    ...
    try {
        ...
        // Poll server 3 times to retrieve newly generated tab
        for (let attempt = 0; attempt < 3; attempt++) {
            await new Promise(r => setTimeout(r, 1000));
            try {
                await fetchArchivesList();
...
// Safely bind bootstrapper when DOM is fully loaded
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initApp);
} else {
    initApp();
