...
// --- GUARANTEED TAGLINE CAROUSEL ENGINE ---
...
async function createNewLedger() {
    ...
    const res = await callBackend('createLedger', { name: nameVal, pin: pinVal, theme: currentTheme, currency: currentCurrency, language: currentLang });
    ...
    try {
        ...
        if (data.status === "success") {
            ...
            [...cleanServerMembers, ...unsavedMembers].forEach(m => {
                if (!memberMap.has(m.toLowerCase())) memberMap.set(m.toLowerCase(), m);
            });
...
function copySettlementSummary() {
    ...
    navigator.clipboard.writeText(summaryText).then(() => alert("Settlement summary copied to clipboard!"));
}
...
function generateLedgerReport() {
    ...
    let report = `====================================================\n               SPENSE LEDGER REPORT                 \n====================================================\n`;
    report += `Ledger Name  : ${currentTab}\nGenerated On : ${new Date().toLocaleString()}\nParticipants : ${ledgerData.members.join(', ') || 'None'}\nTotal Spend  : ${sym}${totalSpent.toFixed(2)}\n====================================================\n\n`;
    report += `--- SETTLEMENT MATRIX ---\n`;
    report += settlement.length > 0 ? settlement.map(s => `• ${s}\n`).join('') : `All balances are currently settled!\n`;
    report += `\n--- ITEMIZED TRANSACTION HISTORY ---\n`;
    report += ledgerData.expenses.map((e, idx) => `${idx + 1}. [${formatToISODate(e.date)}] ${e.desc} (${e.category})\n   Amount: ${sym}${parseFloat(e.amount).toFixed(2)} | Paid By: ${findMemberCanonical(e.paidBy)}\n`).join('\n') || `No expenses recorded.\n`;
...
function renderExpenseFormHeader() {
    ...
    if (editingExpenseId) {
        ...
        actionsContainer.innerHTML = `<button type="button" onclick="window.addExpense()" data-i18n="recordExpenseBtn" class="w-full theme-btn py-3 text-sm font-extrabold cursor-pointer rounded-xl">${t.recordExpenseBtn}</button>`;
    }
}
    ...
    list.innerHTML = ledgerData.expenses.length > 0 ? ledgerData.expenses.map(e => `
        <li class="p-2.5 rounded-xl border border-current/15 flex justify-between items-center bg-current/5 gap-2">
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                ...
                <button type="button" data-id="${e.id}" onclick="window.startEditExpense(this.getAttribute('data-id'))" class="theme-btn px-2.5 py-1 text-[10px] font-black uppercase tracking-wider bg-amber-300 text-slate-900 cursor-pointer">Edit</button>
            </div>
        </li>
...
function renderSettlement() {
    ...
    const txs = calculateSettlement();
    container.innerHTML = txs.length > 0 ? txs.map(t => `<div class="p-2 bg-emerald-50 text-emerald-900 border border-emerald-200 rounded-xl font-bold">${escapeHTML(t)}</div>`).join('') : '<p class="font-bold text-center py-2 text-emerald-600">All balances are currently settled!</p>';
}
